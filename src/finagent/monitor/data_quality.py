...
class DataQualityMonitor:
    ...
    def __init__(self):
        ...
        self.field_stats: dict[str, dict] = defaultdict(
            lambda: {"total": 0, "missing": 0, "invalid": 0}
        )
        self.validation_errors: list[dict] = []
        self.response_stats: dict[str, int] = defaultdict(int)
        self.quality_scores: list[float] = []
        self._issues: list[dict] = []  # 用于存储当前检查的 issues
    ...
    def check_task(self, task: EvalTask) -> dict:
        ...
        if task.task_id in self.task_ids:
            ...
            result["issues"].append(
                {"type": "duplicate_task_id", "message": f"任务ID重复: {task.task_id}"}
            )
        ...
        for field_name in required_fields:
            ...
            if value is None or (isinstance(value, dict) and not value):
                ...
                result["issues"].append({"type": "missing_field", "field": field_name})
        ...
        if task.metadata and "created_at" in task.metadata:
            try:
                ...
                if age_days > 365:  # 数据超过1年
                    result["issues"].append(
                        {"type": "stale_data", "message": f"任务数据已过期 {age_days} 天"}
                    )
    ...
    def check_response(self, response: EvalResponse) -> dict:
        ...
        if response.error:
            ...
            result["issues"].append({"type": "agent_error", "message": response.error})
        elif not response.output and not response.tool_calls:
            self.response_stats["empty"] += 1
            result["is_valid"] = False
            result["issues"].append({"type": "empty_response", "message": "响应无输出且无工具调用"})
        else:
        ...
        if response.tool_calls:
            for i, call in enumerate(response.tool_calls):
                if not isinstance(call, dict):
                    result["issues"].append({"type": "invalid_tool_call_format", "index": i})
                elif "tool_name" not in call:
                    result["issues"].append({"type": "missing_tool_name", "index": i})
    ...
    def generate_report(self) -> DataQualityReport:
        ...
        timeliness = 1.0 - (
            self.response_stats.get("high_latency", 0) / max(1, sum(self.response_stats.values()))
        )
